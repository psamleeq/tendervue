...
export default {
	...
	mounted() {
		if(this.message!=''){
			this.listQuery.zipCode = this.message.zipCode
			this.imageUrl =	this.message.imageUrl
		}
		
	},
	methods: {
		formatFormData(){
			const date = moment(this.message.searchDate).subtract(1911, 'year');
			//日期格式
			this.inputForm.formatDate = date.format("YYYY年MM月DD日").slice(1);
			//民國年份
			this.inputForm.dateYear = date.year()
			//紀錄編號
			this.inputForm.serialNumber = date.format("YYYYMMDD01").slice(1) + String(this.initPage).padStart(2, '0');	
			//行政區
		...
		async getList() {
			...
			let startDate = this.message.startDate;
			let endDate = this.message.endDate;
			this.list = [];
		...
		async createPdf() {
			...
			return new Promise((resolve, reject) => {
				...
				this.pdfDoc.autoTable({
					...
					head: [['工程名稱',`${this.inputForm.dateYear+"年度"+this.inputForm.district+"道路巡查維護修繕成效式契約"}`,'紀錄編號',`${this.inputForm.serialNumber+'-1'}`]],
				...
				this.pdfDoc.autoTable({
					theme: 'plain',
					styles: { font: "edukai", fontSize: 12, lineWidth: 0.1, lineColor: 10 },
					head: [['工程名稱',`${this.inputForm.dateYear+"年度"+this.inputForm.district+"道路巡查維護修繕成效式契約"}`,'紀錄編號',`${this.inputForm.serialNumber+'-2'}`]],
					body: [['施工廠商',`${this.inputForm.companyName}`,'檢查日期',`${this.inputForm.formatDate}`]],
					startY: height-265,
				})
				this.pdfDoc.autoTable({
		 ...
		 async previewPdf(isInstant = false) {
			...
			await this.createPdf().then(() => {
				...
				// console.log(this.pdfDoc.output('dataurlstring'));
