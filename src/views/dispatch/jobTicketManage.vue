<template>
  <div class="app-container job-ticket-manage" v-loading="loading">
    ...
    <div class="filter-container">
      ...
      <el-checkbox v-model="listQuery.filter" style="margin-left: 20px"
        >已退回</el-checkbox
      >
    </div>
    ...
    >
      ...
      >
        <template slot-scope="{ row }">
          <el-button
            v-if="row.DateClose.length == 0 && deviceTypeNow != 4"
            type="danger"
            size="mini"
            style="padding: 5px"
      ...
      >
        <template slot-scope="{ row, column }">
          ...
          <span v-else-if="['Amount'].includes(column.property)">
            ...
            /></el-button>
            <span v-if="row.edit">
              <el-button type="text" @click="editJobTicketAmt(row)"
      ...
      >
        <template slot-scope="{ row }">
          <el-button
            ...
            plain
            size="mini"
            @click="showTicketDetail(row)"
            >檢視</el-button
          >
          <el-button
            v-if="row.DateClose.length == 0"
            class="btn-action"
            type="success"
            plain
            size="mini"
            @click="editJobTicket(row)"
            >編輯</el-button
          >
          <el-button
            v-if="row.DateClose.length == 0"
    ...
    >
      <span slot="title">
        <span v-if="rowActive.revokeType == 1"
          >確認退回 派工單號{{ rowActive.OrderSN }} 至「主任分派」?</span
        >
  ...
  methods: {
    ...
    async createPdf_AC(OrderSN) {
      return new Promise(async (resolve, reject) => {
        ...
        let tonneSUM = 0;
        let areaSUM = 0;
        const splitTable = this.caseSpec.reduce(
          (acc, cur) => {
            tonneSUM += cur.tonne;
    ...
    editJobTicketAmt(row) {
      this.$confirm(
        `確認修改 派工單號${row.OrderSN} 的金額為 ${row.Amount} ?`,
        "確認",
        { showClose: false }
      )
        .then(() => {
        ...
        .then(async (response) => {
          ...
          switch (this.deviceTypeNow) {
            case 1:
              ...
              this.createPdf_AC(row.OrderSN).then(() => {
                this.pdfDoc.save(`維修派工單_${row.OrderSN}.pdf`);
              });
              break;
            case 2:
              this.createPdf_HR(row.OrderSN).then(() => {
                this.pdfDoc.save(`維修派工單_${row.OrderSN}.pdf`);
              });
              break;
            case 3:
              ...
              this.createPdf_FA(row.OrderSN).then(() => {
                this.pdfDoc.save(`維修派工單_${row.OrderSN}.pdf`);
              });
              break;
            case 4:
              this.createPdf_MK(row.OrderSN).then(() => {
                this.pdfDoc.save(`維修派工單_${row.OrderSN}.pdf`);
              });
              break;
          }
