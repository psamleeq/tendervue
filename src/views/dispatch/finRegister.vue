<template>
  <div class="app-container finish-register" v-loading="loading">
    ...
    <div
      v-if="list.length != 0"
      style="width: 300px; margin: 5px 5px 15px 10px"
    >
    ...
    >
      ...
      >
        <template slot-scope="{ row }">
          <span v-if="(deviceTypeNow == 4 && row.IsCancel) || row.edit">
            -
          </span>
          <el-button-group v-else>
            <el-button
              v-if="deviceTypeNow != 4 || row.DeviceType == 4"
              type="danger"
              size="mini"
              style="padding: 5px"
              @click="beforeRemove(row, 1)"
              >主任</el-button
            >
            <el-button
              type="warning"
      ...
      >
        <template slot-scope="{ row, column }">
          <span v-if="['IsMarking'].includes(column.property)">
            <span v-if="row.IsCancel_MK == 1" style="color: #f56c6c"
              >不需施作</span
            >
            <span v-else-if="row.IsMarkingNow == 1 && row.DateClose_MK"
              >{{ formatTime(row.DateClose_MK) }}完工</span
            >
            <span v-else-if="row.IsMarkingNow == 1 && row.OrderSN_MK"
              >派工單{{ row.OrderSN_MK }}</span
            >
            <span v-else-if="row.IsMarkingNow == 1 && row.Contractor_MK"
              >已分派</span
            >
            <el-checkbox
              v-else-if="row.edit && !isAllCompleted"
              v-model="row[column.property]"
              :true-label="1"
              :false-label="0"
            />
            <span v-else-if="row.IsMarkingNow == 1"
      ...
      <!-- 道路、熱再生 -->
      <el-table-column
        v-if="[1, 2].includes(deviceTypeNow)"
        label="算式"
        width="255"
        align="center"
      >
        <template slot-scope="{ row }">
          <span v-if="row.edit">
            ...
            >
              ...
              >
              <el-col :span="20"
                ><el-input v-model="row.MillingFormula" @change="calArea(row)"
              /></el-col>
            </el-row>
            <el-row v-else :gutter="5" type="flex" align="middle">
              ...
              >
              <el-col :span="8"
                ><el-input v-model="row.MillingLength" @change="calArea(row)"
              /></el-col>
              <el-col :span="2" style="line-height: 36px"> ✕ </el-col>
              <el-col :span="8"
                ><el-input v-model="row.MillingWidth" @change="calArea(row)"
              /></el-col>
            </el-row>
          </span>
          <span v-else>
            <span v-if="row.MillingFormula != '0'">{{
              row.MillingFormula
            }}</span>
      ...
      >
        <template slot-scope="{ row }">
          <span v-if="row.edit">
            <el-row
              v-for="key in options.workmemoOrder.filter(
                (key) => key != 'uNotes'
              )"
              :gutter="5"
              :key="key"
              ...
              <el-col :span="16">
                <el-input v-model.number="row[key]" size="mini">
                  <el-button
                    ...
                    @click="beforeMImgEdit(row, key)"
                    >上傳照片 ({{ row[`${key}Img`].length }})</el-button
                  >
                </el-input>
              </el-col>
            </el-row>
          ...
          <el-row :gutter="5" v-else>
            <span
              v-if="
                options.workmemoOrder.filter(
                  (key) =>
                    row.uNotes.length > 0 || (key != 'uNotes' && row[key] != 0)
                ).length != 0
              "
            >
              <el-row :gutter="5">
                ...
                >
                  ...
                  <el-col :span="6" class="item-content"
                    ...
                    <span v-if="row[`${key}Img`].length > 0">
                      <el-popover
                        v-if="row[`${key}Img`].length > 0"
                        popper-class="imgHover"
                        placement="top"
                        trigger="hover"
                      >
                        <div
                          v-for="(imgSpec, index) of row[`${key}Img`]"
                          :key="`img_${index}`"
                        >
                          <el-image
                        ...
                        <span slot="reference">
                          <i class="el-icon-picture" style="color: #409eff" />{{
                            row[`${key}Img`].length
                          }}
                        </span>
                      </el-popover>
                    ...
                    <span v-else
                      ><i class="el-icon-picture" style="color: #f56c6c" />{{
                        row[`${key}Img`].length
                      }}</span
                    >)
                  </el-col>
      ...
      >
        <template slot-scope="{ row }">
          <span v-if="row.edit">
            <el-select
              v-model="row.MillingDepth"
              size="mini"
              popper-class="type-select"
            >
              <el-option
                v-for="value in options.depthArr"
                :key="value"
                :label="value"
                :value="value"
              />
        ...
        <template slot-scope="{ row }">
          <span v-if="row.edit">
            ...
            <el-row :gutter="5">
              <el-col :span="8">
                <el-select
                  v-model="row.SamplingL1Detail.Aggregate"
                  size="mini"
                  popper-class="type-select"
                  :disabled="row.SamplingL1 == 0"
                >
                  <el-option
                    v-for="(value, type) in options.sampling.typeMap"
                    :key="type"
                    :label="value"
                    :value="type"
                  />
              ...
              <el-col :span="8">
                <el-input
                  v-model="row.SamplingL1Detail.Amount"
                  size="mini"
                  :disabled="row.SamplingL1 == 0"
                />
              </el-col>
              <el-col :span="8">
                <el-select
                  v-model="row.SamplingL1Detail.Unit"
                  size="mini"
                  popper-class="type-select"
                  :disabled="row.SamplingL1 == 0"
                >
                  <el-option
                    v-for="value in options.sampling.unitArr"
                    :key="value"
                    :label="value"
                    :value="value"
                  />
            ...
            <el-row :gutter="5">
              <el-col :span="8">
                <el-select
                  v-model="row.SamplingL2Detail.Aggregate"
                  size="mini"
                  popper-class="type-select"
                  :disabled="row.SamplingL2 == 0"
                >
                  <el-option
                    v-for="(value, type) in options.sampling.typeMap"
                    :key="type"
              ...
              <el-col :span="8">
                <el-input
                  v-model="row.SamplingL2Detail.Amount"
                  size="mini"
                  :disabled="row.SamplingL2 == 0"
                />
              </el-col>
              <el-col :span="8">
                <el-select
                  v-model="row.SamplingL2Detail.Unit"
                  size="mini"
                  popper-class="type-select"
                  :disabled="row.SamplingL2 == 0"
                >
          ...
          <span v-else>
            <el-row :gutter="5">
              ...
              <el-col :span="3" class="item-content">{{
                row.Aggregate34
              }}</el-col>
            ...
            <el-row :gutter="5" v-if="row.SamplingL1">
              ...
              <el-col :span="6">{{
                options.sampling.typeMap[row.SamplingL1Detail.Aggregate]
              }}</el-col>
              <el-col :span="6"
                ><span class="item-content">{{
                  row.SamplingL1Detail.Amount
                }}</span>
                {{ row.SamplingL1Detail.Unit }}</el-col
              >
            </el-row>
            ...
            <el-row :gutter="5" v-if="row.SamplingL2">
              ...
              <el-col :span="6">{{
                options.sampling.typeMap[row.SamplingL2Detail.Aggregate]
              }}</el-col>
              <el-col :span="6"
                ><span class="item-content">{{
                  row.SamplingL2Detail.Amount
                }}</span>
                {{ row.SamplingL2Detail.Unit }}</el-col
              >
            </el-row>
          </span>
        </template>
      ...
      <!-- 設施、標線 -->
      <el-table-column
        v-if="[3, 4].includes(deviceTypeNow)"
        label="實際數量"
        width="140"
        align="center"
      >
        <template slot-scope="{ row }">
          <span v-if="deviceTypeNow == 4 && row.IsCancel" style="color: #f56c6c"
            >不需施作</span
          >
          <el-button-group v-else-if="!row.edit">
            <el-button
              ...
              :type="
                row.DateRecord == null &&
                row.TaskRealGroup == row.TaskRealGroup_Plan
                  ? 'success'
                  : 'info'
              "
              :plain="!row.Content.length != 0"
              size="mini"
      ...
      <el-table-column label="動作" align="center">
        <template slot-scope="{ row }">
          <el-button-group v-if="!row.edit">
            ...
            <el-button
              v-if="!isAllCompleted && deviceTypeNow == 4"
              :type="row.IsCancel ? 'success' : 'danger'"
              size="mini"
              plain
              @click="finishCancel(row, Number(!row.IsCancel))"
            >
              {{ row.IsCancel ? "恢復施作" : "不需施作" }}
            </el-button>

            <!-- <el-button v-if="[3,4].includes(deviceTypeNow)" size="mini" @click="toggleExpand(row)">詳情</el-button> -->
            <el-button type="info" size="mini" @click="showDetail(row)"
              >檢視</el-button
            >
          </el-button-group>
          <el-button-group v-else>
            <el-button
              type="primary"
              size="mini"
              @click="finishRegisterSpec(row, false)"
              >確定</el-button
            >
            <el-button
              size="mini"
            ...
            >
              ...
              >
                <template slot-scope="{ row: rowSpec, column: colSpec }">
                  <span
                    v-if="['ItemPaint', 'ItemType'].includes(colSpec.property)"
                    >{{
                      options[`${colSpec.property}Map`][
                        rowSpec[colSpec.property]
                      ].name
                    }}</span
                  >
    ...
    <el-button
      class="btn-previewPdf"
      :type="isAllCompleted ? 'danger' : 'success'"
      icon="el-icon-s-claim"
      :disabled="list.length == 0 || isAllCompleted"
    ...
    <el-dialog
      v-loading="loading"
      width="520px"
      :title="`照片上傳(${options.workmemo[imgUploadKey]})`"
      :visible.sync="showMImgUploadDialog"
      :close-on-click-modal="false"
      :close-on-press-escape="false"
      :show-close="false"
    >
      <el-upload
        :key="`${imgUploadKey}Img`"
        action="#"
        :auto-upload="false"
        list-type="picture-card"
        :file-list="rowActive[`${imgUploadKey}Img`]"
    ...
    <el-dialog
      ...
      width="520px"
      title="照片上傳"
      :visible.sync="showImgUploadDialog"
      :close-on-click-modal="false"
      :close-on-press-escape="false"
      :show-close="false"
    >
      <div v-if="deviceTypeNow == 3" class="filter-container">
        <el-select
          class="filter-item"
          v-model.number="rowActive.imgId"
          filterable
          placeholder="請選擇"
          popper-class="type-select"
          style="width: 120px"
        >
          <el-option
            v-for="image in restoredImgMapFilter()"
            :key="image.Id"
            :value="Number(image.Id)"
            :label="image.ImgName"
          />
        </el-select>
        <el-button
      ...
      <el-row
        v-for="(val, col) in restoredImgRegularFilter()"
        :key="col"
        type="flex"
        align="middle"
      >
        <el-col :span="7">
          ...
          <el-button
            ...
            @click="removePicType(col)"
            >刪除</el-button
          >
        </el-col>
        <el-col
          :span="17"
          style="border-bottom: 1px solid #ebeef5; padding: 0 5px 10px 5px"
        >
          <el-upload
            action="#"
            :auto-upload="false"
            list-type="picture-card"
            :file-list="
              Number(col) ? rowActive.Image[col] : rowActive[`${col}Img`]
            "
            :on-change="(file, fileList) => handleChange(file, fileList, col)"
            :on-preview="(file) => handlePreview(file, col)"
            :on-remove="(file, fileList) => handleRemove(file, fileList, col)"
            @click.native="imgUploadKey = col"
          >
            <i class="el-icon-plus" />
            <!-- <el-button slot="trigger" type="info">選取</el-button> -->
            <div slot="tip" class="el-upload__tip">
    ...
    >
      ...
      >
        ...
        <el-col :span="23">
          <el-table
            ...
            show-summary
            :summary-method="
              (param) => getSummaries(param, rowActive.Content_Plan)
            "
            style="width: 100%"
          >
            <el-table-column
      ...
      <el-row type="flex" align="middle" justify="center">
        <el-col v-if="deviceTypeNow == 3" class="task-table-text" :span="1"
          >實際數量</el-col
        >
        <el-col :span="deviceTypeNow == 3 ? 23 : 24">
          <div v-if="deviceTypeNow == 3" class="filter-container">
            <el-select
              class="filter-item"
              v-model.number="listQuery.groupSN"
              filterable
              placeholder="請選擇"
              popper-class="type-select"
              style="width: 500px"
            >
              <el-option
                v-for="kit in options.kitArr"
                :key="kit.SerialNo"
                :value="Number(kit.SerialNo)"
                :label="kit.GroupName"
              />
            </el-select>
            <el-button
          ...
          >
            ...
            >
              <template slot-scope="{ row, column }">
                ...
                <span v-if="deviceTypeNow == 3">
                  <span
                    v-if="['number'].includes(column.property)"
                  ...
                  <span
                    v-else-if="
                      ['UnitSN'].includes(column.property) && row.isAdd
                    "
                  >
                    <span v-if="row.isAdd || row.isEdit">
                ...
                <span v-else-if="deviceTypeNow == 4">
                  <span
                    v-if="
                      ['MillingFormula', 'number'].includes(column.property)
                    "
                    style="display: inline-flex; align-items: center"
                  >
                    <span v-if="row.isAdd || row.isEdit">
                  ...
                  <span
                    v-else-if="
                      ['ItemPaint', 'ItemType'].includes(column.property)
                    "
                  >
                    <span v-if="row.isAdd">
                      <el-select
                      ...
                      >
                        <el-option
                          v-for="(val, id) in options[`${column.property}Map`]"
                          :key="id"
                          :value="Number(id)"
                          :label="val.name"
                        />
                    ...
                    <span v-else>{{
                      options[`${column.property}Map`][row[column.property]]
                        .name
                    }}</span>
                  </span>
                  <span v-else>{{ row[column.property] }}</span>
            ...
            <el-table-column label="動作" align="center" :min-width="30">
              <template slot-scope="{ row, $index }">
                <span v-if="row.isAdd">
                  <el-button type="success" size="mini" @click="addItem"
                    >新增</el-button
                  >
                </span>
                ...
                <span v-else>
                  <el-button type="danger" size="mini" @click="delItem($index)"
                    >刪除</el-button
                  >
                </span>
              </template>
      ...
      <span v-if="deviceTypeNow == 3">
        ...
        <el-row type="flex" align="middle" justify="center">
          ...
          <el-col class="detail-note plan" :span="23">
            <el-input
              placeholder="請輸入"
              v-model="rowActive.KitNotes_Plan.DesignDetail"
              disabled
            >
            ...
            <el-input
              placeholder="請輸入"
              v-model="rowActive.KitNotes_Plan.DesignDesc"
              disabled
            >
            ...
            <el-input
              placeholder="請輸入"
              v-model="rowActive.KitNotes_Plan.DesignWorker"
              disabled
            >
        ...
        <el-row type="flex" align="middle" justify="center">
          ...
          <div class="detail-note">
            <el-input
              placeholder="請輸入"
              v-model="rowActive.KitNotes.DesignDetail"
            >
              <template slot="prepend">施作數量</template>
            </el-input>
            <el-input
              placeholder="請輸入"
              v-model="rowActive.KitNotes.DesignDesc"
            >
              <template slot="prepend">施工方式</template>
              <el-checkbox slot="append" v-model="rowActive.notesSync"
                >更新「工程概述」</el-checkbox
              >
            </el-input>
            <el-input
              placeholder="請輸入"
              v-model="rowActive.KitNotes.DesignWorker"
            >
              <template slot="prepend">施作人力</template>
            </el-input>
          </div>
    ...
    >
      <span slot="title">
        <span v-if="rowActive.revokeType == 1"
          >確認退回 案件編號{{ rowActive.CaseNo }} 至「主任分派」?</span
        >
        <span v-else-if="rowActive.revokeType == 2"
          >確認退回 案件編號{{ rowActive.CaseNo }} 至「製作派工單」?</span
        >
      </span>
      <div v-if="rowActive.revokeType == 1">
        原因:
        <el-select v-model="rowActive.revokeReason">
          <el-option
            v-for="(name, key) in options.reasonType"
            :key="key"
            :label="name"
            :value="Number(key)"
          />
        ...
        備註:
        <el-input
          class="revokeReasonNote"
          type="textarea"
          :rows="2"
          placeholder="請輸入備註"
          v-model="revokeTextarea"
        >
        </el-input>
      </div>
...
export default {
  ...
  computed: {
    headersFilter() {
      ...
      Object.keys(this.headers).forEach((key) => {
        ...
        if (!props.hasOwnProperty("deviceTypeFilter"))
          headersFilter[key] = props;
        else if (props.deviceTypeFilter.includes(this.deviceTypeNow))
          headersFilter[key] = props;
      });
    ...
    caseSum() {
      return this.list.reduce(
        (acc, cur) => {
          ...
          acc.tonneSUM +=
            Math.round(cur.MillingArea * cur.MillingDepth * 0.01 * 2.25 * 10) /
            10;
          return acc;
        },
  ...
  created() {
    ...
    getSCTypeItemMap().then((response) => {
      this.options.ItemTypeMap = response.data.SCType2Map;
    });
    getRestoredImgMap().then((response) => {
      this.options.restoredImgMap = response.data.restoredImgMap;
    });
  },
  mounted() {
  ...
  methods: {
    ...
    adjustTableLayout() {
      const expandTableSumList = document.querySelectorAll(
        ".taskTable .el-table__footer-wrapper tr"
      );
    ...
    tableMinWidth(key) {
      if (this.deviceTypeNow == 3)
        return ["TaskName"].includes(key)
          ? 100
          : ["UnitSN", "TaskUnit", "TaskPrice"].includes(key)
          ? 20
          : 30;
      else if (this.deviceTypeNow == 4)
        return ["MillingFormula"].includes(key)
          ? 90
          : ["MillingArea"].includes(key)
          ? 20
          : 50;
      else return null;
    },
    detailAmount(content) {
      return content.reduce(
        (acc, cur) => (acc += cur.number * Number(cur.TaskPrice)),
        0
      );
    },
    getSummaries(param, content) {
      ...
      columns.forEach((column, index) => {
        ...
        if (![1, 2].includes(index)) {
          if (column.property == "TaskPrice")
            sums[index] = this.detailAmount(content).toLocaleString();
        }
      });
      return sums;
    },
          ...
          .then((response) => {
            if (response.data.list.length == 0) {
              ...
              this.list.forEach((l) => {
                ...
                for (const mKey of Object.keys(this.options.workmemo).filter(
                  (key) => key != "uNotes"
                ))
                  if (
                    l.hasOwnProperty(`${mKey}Img`) &&
                    Object.keys(l[`${mKey}Img`]).length > 0
                  )
                    this.$set(
                      l,
                      `${mKey}Img`,
                ...
                for (const key of Object.keys(this.restoredImgRegularFilter()))
                  if (
                    l.hasOwnProperty(`${key}Img`) &&
                    Object.keys(l[`${key}Img`]).length > 0
                  )
                    this.$set(
                      l,
                      `${key}Img`,
                ...
                if (l.hasOwnProperty("SamplingL1Detail")) {
                  l.SamplingL1Detail =
                    Object.keys(l.SamplingL1Detail).length == 0
                      ? Object.assign(
                          {},
                          { Aggregate: "", Amount: 0, Unit: "" }
                        )
                      : l.SamplingL1Detail;
                }
                ...
                if (l.hasOwnProperty("SamplingL2Detail")) {
                  l.SamplingL2Detail =
                    Object.keys(l.SamplingL2Detail).length == 0
                      ? Object.assign(
                          {},
                          { Aggregate: "", Amount: 0, Unit: "" }
                        )
                ...
                if (l.MillingAreaArr)
                  l.MillingArea = l.MillingAreaArr.reduce(
                    (acc, cur) => acc + cur,
                    0
                  );
    ...
    getTaskDetail(row) {
      return new Promise((resolve) => {
        row.Content = [];
        getTaskReal({
          taskRealGroup: row.TaskRealGroup,
          taskRealGroup_Plan: row.TaskRealGroup_Plan,
        })
          .then((response) => {
            row.Content = response.data.list.filter(
              (l) => l.GroupId == row.TaskRealGroup
            );
            row.Content_Plan = response.data.list.filter(
              (l) => l.GroupId == row.TaskRealGroup_Plan
            );
            resolve();
          })
          .catch((err) => (this.loading = false));
    ...
    calArea(row) {
      ...
      if (
        (row.editFormula == undefined &&
          row.MillingFormula &&
          row.MillingFormula != "0" &&
          row.MillingFormula.length != 0) ||
        row.editFormula
      ) {
        for (const key in replaceObj)
          row.MillingFormula = row.MillingFormula.replaceAll(
            key,
            replaceObj[key]
          );
        row.MillingArea = regex.test(row.MillingFormula)
          ? Math.round(
              new Function(`return ${row.MillingFormula} * ${number}`)() * 100
            ) / 100
          : 0;
      } else
        row.MillingArea =
          Math.round(row.MillingLength * row.MillingWidth * number * 100) / 100;
    },
    caseFilterList(list) {
      // console.log(list);
      let caseFilterList = [];
    ...
    restoredImgRegularFilter() {
      ...
      Object.keys(this.options.restoredImgRegular).forEach((key) => {
        ...
        if (!props.hasOwnProperty("deviceTypeFilter"))
          restoredImgRegularFilter[key] = props;
        else if (props.deviceTypeFilter.includes(this.deviceTypeNow))
          restoredImgRegularFilter[key] = props;
      });
    ...
    restoredImgMapFilter() {
      return this.options.restoredImgMap.filter(
        (image) =>
          !Object.keys(this.imgTypePlus).includes(String(image.Id)) &&
          (image.DeviceType == 0 || image.DeviceType == this.deviceTypeNow)
      );
    },
    ...
    handleRemove(file, fileList, key = this.imgUploadKey) {
      ...
      if (file.status == "success") this.imgObj[key].remove.push(file);
        ...
        this.imgObj[key].add = this.imgObj[this.imgUploadKey].add.filter(
    ...
    beforeMImgEdit(row, imgUploadKey) {
      for (const row of this.list)
        this.$refs.caseTable.toggleRowExpansion(row, false);
    ...
    beforeImgEdit(row) {
      ...
      this.imgObj = {};
      this.imgTypePlus = {};
      for (const imgId in row.Image)
        this.imgTypePlus[imgId] = {
          name: this.restoredImgMapFilter().filter((img) => img.Id == imgId)[0]
            .ImgName,
          isAdd: true,
        };
      for (const key of Object.keys(this.restoredImgRegularFilter()))
    ...
    addPicType() {
      ...
      this.imgTypePlus[this.rowActive.imgId] = {
        name: this.restoredImgMapFilter().filter(
          (img) => img.Id == this.rowActive.imgId
        )[0].ImgName,
        isAdd: true,
      };
    ...
    removePicType(key) {
      this.imgObj[key].remove = this.rowActive.Image[key]
        ? this.rowActive.Image[key].filter((file) => file.status == "success")
        : [];
      this.imgObj[key].add = [];
    ...
    submitMImgUpload() {
      if (
        this.imgObj[this.imgUploadKey].add.length == 0 &&
        this.imgObj[this.imgUploadKey].remove.length == 0
      ) {
      ...
      for (const file of this.imgObj[this.imgUploadKey].add.filter(
        (f) => f.raw
      ))
        uploadForm.append("fileAddList", file.raw);
      for (const file of this.imgObj[this.imgUploadKey].remove.filter(
        (f) => f.url
      ))
        uploadForm.append("fileRemoveList", file.url);

      finImgUpload(uploadForm)
        .then((response) => {
          if (response.statusCode == 20000) {
            ...
            this.showMImgUploadDialog = false;
            this.rowActive[`${this.imgUploadKey}Img`] = response.imgList.map(
              (url) => ({
                name: url.split("/").slice(-1),
                status: "success",
                url,
    ...
    async submitImgUpload() {
      for (const [index, key] of Object.keys(this.imgObj).entries()) {
        if (
          this.imgObj[key].add.length == 0 &&
          this.imgObj[key].remove.length == 0
        ) {
          if (index == Object.keys(this.imgObj).length - 1)
        ...
        for (const file of this.imgObj[key].add.filter((f) => f.raw))
          uploadForm.append("fileAddList", file.raw);
        for (const file of this.imgObj[key].remove.filter((f) => f.url))
          uploadForm.append("fileRemoveList", file.url);

        await finImgUpload(uploadForm)
          .then((response) => {
            const imgName = this.restoredImgRegularFilter()[key]
              ? this.restoredImgRegularFilter()[key].name
              : this.options.restoredImgMap.filter(
                  (image) => image.Id == key
                )[0].ImgName;
            if (response.statusCode == 20000) {
              this.$message({
      ...
      getTaskGroupDetail({
        ...
        .then((response) => {
          ...
          itemArr.forEach((itemAdd) => {
            const detailFilter = this.detail.filter(
              (itemNow) => itemNow.UnitSN == itemAdd.UnitSN
            );
          ...
          const isReplaceNote =
            Object.values(this.rowActive.KitNotes).filter(
              (val) => val && val.length != 0
            ).length == 0;
          if (isReplaceNote)
            this.rowActive.KitNotes = this.options.kitArr
              .filter((kit) => kit.SerialNo == this.listQuery.groupSN)
              .map((kit) => ({
                DesignDetail: kit.DesignDetail,
                DesignDesc: kit.DesignDesc,
                DesignWorker: kit.DesignWorker,
              }))[0];
    ...
    async getKitItem(row) {
      return new Promise((resolve) => {
        this.loading = true;
        const rowActive =
          row.SerialNo != undefined ? row : this.newItem[this.deviceTypeNow];
    ...
    async addMarkerItem() {
      ...
      if (
        !newItem.ItemPaint ||
        !newItem.ItemType ||
        newItem.MillingFormula.length == 0 ||
        newItem.number == "0"
      ) {
    ...
    finishRegisterSpec(row = this.rowActive, editContent = true) {
      this.$confirm(`確認 案件編號${row.CaseNo} 資料登錄?`, "確認", {
        showClose: false,
      })
        .then(() => {
          this.loading = true;
          ...
          let caseSpec = JSON.parse(
            JSON.stringify(this.caseFilterList([row])[0])
          );
          ...
          if ([1, 2].includes(this.deviceTypeNow)) {
            ...
            if (
              caseSpec.MillingFormula &&
              caseSpec.MillingFormula != "0" &&
              caseSpec.MillingFormula.length != 0
            ) {
            ...
            if (this.deviceTypeNow == 1) {
              ...
              if (caseSpec.SamplingL1 && caseSpec.SamplingL1 == 1)
                caseSpec.SamplingL1Detail = JSON.stringify(
                  row.SamplingL1Detail
                );
              if (caseSpec.SamplingL2 && caseSpec.SamplingL2 == 1)
                caseSpec.SamplingL2Detail = JSON.stringify(
                  row.SamplingL2Detail
                );
            }
            ...
            if (this.deviceTypeNow == 3) {
              ...
              caseSpec.Notes =
                editContent && row.notesSync
                  ? row.KitNotes.DesignDesc
                  : row.Notes;
            } else if (this.deviceTypeNow == 4)
              caseSpec.Content = JSON.stringify(
                editContent ? this.detail : row.Content
              );
          }
    ...
    finishRegister() {
      this.$confirm(`確認將 派工單號${this.orderSNNow} 完工登錄?`, "確認", {
        showClose: false,
      })
        .then(() => {
          const tableSelectFilter = this.list.filter(
            (caseSpec) =>
              !Boolean(caseSpec.IsCancel) &&
              caseSpec.Content &&
              caseSpec.Content.length == 0
          );
          const condition =
            [3, 4].includes(this.deviceTypeNow) &&
            tableSelectFilter.length != 0;
          if (condition) {
            this.$message({
              type: "error",
              message: `案件編號${tableSelectFilter
                .map((caseSpec) => caseSpec.CaseNo)
                .join("、")} 未填入「登錄數量」`,
            });
          } else {
            this.loading = true;
            let caseList = JSON.parse(
              JSON.stringify(this.caseFilterList(this.list))
            );
    ...
    removeDispatch() {
      ...
      if (this.rowActive.revokeType == 1) {
        this.rowActive.revokeReason =
          this.options.reasonType[this.rowActive.revokeReason];
      }
      let mergeRevokeReason =
        String(this.rowActive.revokeReason) + "-" + String(this.revokeTextarea);
      // console.log(mergeRevokeReason);
      revokeDispatch({
    ...
    finishCancel(row, isCancel) {
      const confirmText =
        isCancel == 1
          ? `確認將 案件編號${row.CaseNo} 標記為「不需施作」?`
          : `確認將 案件編號${row.CaseNo} 恢復成「施作狀態」?`;
