...
export default {
  ...
  async created() {
    ...
    getGuildMap().then((response) => {
      ...
      const guildFilter = Object.keys(this.options.guildMap).filter(
        (key) => this.options.guildMap[key] == "泓景"
      );
      this.listQuery.contractor = this.contractorNow =
        guildFilter.length > 0 ? Number(guildFilter[0]) : null;
    });
  },
  mounted() {
  ...
  methods: {
          ...
          if (response.data.list.length == 0) {
            ...
            this.list.forEach((l, i) => {
              ...
              this.$set(
                l,
                "tonne",
                Math.round(l.MillingArea * l.MillingDepth * 0.01 * 2.25 * 10) /
                  10
              );
            });
          }
    ...
    subDispatch() {
      let confirmText = `確認分派 案件編號${this.tableSelect
        .map((caseSpec) => caseSpec.CaseNo)
        .join("、")} `;
      if (this.deviceTypeNow != 3)
        confirmText += `給 ${
          this.options.guildMap[this.listQuery.contractor]
        } `;
      confirmText += `?`;

      this.$confirm(confirmText, "確認", { showClose: false })
        .then(() => {
          const tableSelectFilter = this.tableSelect.filter(
            (caseSpec) => caseSpec.TaskRealGroup == 0
          );
          this.loading = true;
          const caseList = JSON.parse(
            JSON.stringify(this.caseFilterList(this.tableSelect))
          );
          setJobSubCTR({
