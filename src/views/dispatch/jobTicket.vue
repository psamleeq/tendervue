      ...
      <div class="filter-item">
        <div v-if="listQuery.filterType == 1" class="select-contract">
          <el-select v-model="listQuery.filterType" popper-class="type-select">
            <el-option
              v-for="(name, type) in options.filterType"
              :key="type"
              :label="name"
              :value="Number(type)"
            />
          ...
          <el-select
            v-model="listQuery.tenderId"
            class="tender-select"
            placeholder="請選擇"
            popper-class="type-select tender"
            clearable
            @clear="listQuery.tenderId = null"
          >
            <el-option
              v-for="(name, id) in options.tenderMap"
              :key="id"
              :value="id"
              :label="name"
            />
    ...
    >
      <el-table-column label="順序" width="60" align="center" fixed>
        <template slot-scope="{ row }">
          <el-select
            v-model="row.OrderIndexNew"
            placeholder="請選擇"
            size="mini"
            :disabled="tableSelect.length <= 1"
            @change="changeOrder(row)"
          >
            <el-option
              v-for="index in Array.from(
                { length: tableSelect.length },
                (_, i) => i + 1
              ).filter((i) => i != row.OrderIndex)"
              :key="`order_${index}`"
              :label="index"
              :value="index"
            />
      ...
      <el-table-column prop="index" width="60" align="center" fixed>
        <template slot="header">
          ...
          <el-checkbox
            ...
            :disabled="list.length == 0 || tableSelect.length == 0"
            :key="`checkAll_${checkAll}`"
          />
        </template>
        <template slot-scope="{ row }">
          <el-checkbox
            v-model="checkList[row.index - 1]"
            :key="`list_${row.index}`"
            style="margin-right: 5px"
            @change="cellCheckBox(row)"
          />
          <span>{{ row.index }}</span>
        </template>
      </el-table-column>
        ...
        <template slot-scope="{ row }">
          <el-button-group>
            <el-button
              ...
              size="mini"
              @click="toggleExpand(row, 'selectTable')"
              >詳情</el-button
            >
              ...
              <el-table-column
                ...
                :min-width="
                  ['itemName'].includes(key)
                    ? 100
                    : ['itemId', 'unit', 'uPrice'].includes(key)
                    ? 18
                    : 30
                "
                :label="value.name"
      ...
      <el-table-column width="60" align="center" fixed>
        <template slot="header">
          ...
          <el-checkbox
            ...
            :indeterminate="indeterminate"
            :key="`checkAll_${checkAll}`"
            :disabled="list.length == 0 || listFilter.length == 0"
          />
        </template>
        <template slot-scope="{ row }">
          <el-checkbox
        ...
        <template slot-scope="{ row }">
          <el-button-group>
            <el-button
              v-if="deviceTypeNow == 3"
              size="mini"
              @click="toggleExpand(row, 'confirmTable')"
              >詳情</el-button
            >
            <!-- <el-button v-if="deviceTypeNow == 3" type="success" size="mini" @click="beforeEdit(row)">設計</el-button> -->
            <el-button type="info" size="mini" @click="showDetail(row)"
...
export default {
  ...
  computed: {
    ...
    checkAll: {
      get() {
        return (
          this.list.length > 0 && this.tableSelect.length == this.list.length
        );
      },
      set(newVal) {
    ...
    indeterminate() {
      return (
        ...
        this.tableSelect.length < this.list.length
      );
    },
  },
  watch: {
  ...
  methods: {
    async handleCheckedChange(val) {
      ...
      if (this.tableSelect.length == this.list.length)
        this.tableSelect.forEach((_, index) =>
          this.$set(this.checkList, index, true)
        );
      if (this.tableSelect.length == 0)
        this.checkList = this.checkList.map(() => false);
    ...
    cellCheckBox(row) {
      ...
      if (this.checkList[row.index - 1]) this.tableSelect.push(row);
        ...
        this.tableSelect = this.tableSelect.filter(
          (selectRow) => selectRow.index != row.index
        );
        ...
        .then(() => {
          ...
          this.$nextTick(() => {
            ...
            const expandTableSumList = document.querySelectorAll(
              ".expandTable .el-table__footer-wrapper tr"
            );
    ...
    resetOrder() {
      for (const [index, row] of this.tableSelect.entries())
        row.OrderIndex = row.OrderIndexNew = index + 1;
    },
    imgPreload() {
      //img preload
      this.imgDOMObj = {};
    ...
    getList(showMsg = true) {
      if (!Number(this.listQuery.contractor)) {
        ...
        getJobTicket({
          ...
          tenderId:
            this.listQuery.filterType == 1 ? this.listQuery.tenderId : null,
          reportSN:
            this.listQuery.filterType == 2 && this.listQuery.filterStr
              ? this.listQuery.filterStr
              : null,
          keywords:
            this.listQuery.filterType == 3 && this.listQuery.filterStr
              ? this.listQuery.filterStr
              : null,
          ...
          .then((response) => {
            if (response.data.list.length == 0) {
              ...
              this.list.forEach((l, i) => {
                ...
                l.DateDeadline =
                  l.DateDeadline == null ? "" : this.formatDate(l.DateDeadline);
                for (const col of [
                  "MillingDepth",
                  "MillingLength",
                  "MillingWidth",
                ...
                this.$set(
                  ...
                  "tonne",
                  Math.round(
                    l.MillingArea * l.MillingDepth * 0.01 * 2.25 * 10
                  ) / 10
                );
              });
    ...
    getTaskDetail(row) {
      return new Promise((resolve) => {
        row.Content = [];
        getTaskReal({ taskRealGroup: row.TaskRealGroup })
          .then((response) => {
    ...
    async createPdf_header() {
      return new Promise((resolve, reject) => {
        ...
        const subTitle =
          this.deviceTypeNow == 1
            ? "AC"
            : this.options.deviceType[this.deviceTypeNow];
        this.pdfDoc.setFontSize(this.pdfSetting.fontSize + 4);
        this.pdfDoc.setTextColor("#000000");
        this.pdfDoc.setCharSpace(2);
        this.pdfDoc.text(`道路(${subTitle}) 維修派工單`, width / 2, 20, {
          align: "center",
        });

        const today = `中華民國${moment().year() - 1911}年${moment().format(
          "MM年DD日"
        )}`;
    ...
    async createPdf_footer() {
      return new Promise((resolve, reject) => {
        ...
        for (
          let pageNo = 1;
          pageNo <= this.pdfDoc.internal.getNumberOfPages();
          pageNo++
        ) {
          this.pdfDoc.setPage(pageNo);
          this.pdfDoc.text(
            `${pageNo} of ${this.pdfDoc.internal.getNumberOfPages()}`,
            width / 2,
            height - 10,
            { align: "center" }
          );
    ...
    async createPdf_AC() {
      return new Promise(async (resolve, reject) => {
        ...
        let tonneSUM = 0;
        let areaSUM = 0;
        const splitTable = this.tableSelect.reduce(
          (acc, cur) => {
            tonneSUM += cur.tonne;
        ...
        for (const [pageIndex, table] of splitTable.entries()) {
          this.pdfDoc.addPage();
          while (pageIndex == 0 && this.pdfDoc.internal.getNumberOfPages() > 1)
          ...
          this.pdfDoc.autoTable({
            columns: [
              ...
              {
                header: String(Math.floor(areaSUMHeader * 10) / 10),
                dataKey: "areaSUMHeader",
              },
              { header: "總噸數", dataKey: "tonneSUMTitle" },
              {
                header: String(Math.floor(tonneSUMHeader * 10) / 10),
                dataKey: "tonneSUMHeader",
              },
            ],
            theme: "plain",
            styles: {
          ...
          this.pdfDoc.autoTable({
            ...
            body: table.map((l, i) => ({
              ...
              MillingFormula:
                l.MillingFormula != "0"
                  ? l.MillingFormula
                  : `${l.MillingLength}*${l.MillingWidth}`,
          ...
          const splitImgTable = table.reduce(
            (acc, cur) => {
          ...
          for (const [imgIndex, imgTable] of splitImgTable.entries()) {
            ...
            this.pdfDoc.autoTable({
              head: [
                imgTable.map(
                  (l, i) =>
                    `${i + 1 + 4 * imgIndex + 8 * pageIndex} - ${l.CaseNo}`
                ),
              ],
              ...
              startY:
                this.pdfDoc.lastAutoTable.finalY + 8 * Number(imgIndex == 0),
              pageBreak: "avoid",
            });
          }
        }
              ...
              .map((l, i) => {
                ...
                for (let j = 0; j < 4; j++) {
                  ...
                  rowArr.push({
                    ...
                    dateContent: {
                      content:
                        dateContents[j] != undefined ? l[dateContents[j]] : "",
                    },
    ...
    previewPdf(isInstant = false) {
      ...
      this.createPdf().then(() => {
        const schemas = Array.from(
          { length: this.pdfDoc.internal.getNumberOfPages() },
          () => ({
            OrderSN: {
              type: "text",
              position: {
    ...
    handleDownload(filename) {
      generate({
        template: this.viewer.getTemplate(),
        inputs: this.viewer.getInputs(),
        options: { font: this.viewer.getFont() },
      }).then((pdf) => {
        // console.log(pdf);
