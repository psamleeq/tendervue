		...
		<el-table ref="multipleTable" empty-text="目前沒有資料" :data="list" border fit :header-cell-style="{ 'background-color': '#F2F6FC' }" style="width: 100%">
			...
			>
				<template slot-scope="{ row, column }">
					<span v-if="row[column.property]  == 'CaseNo'">
						<span v-if="row.edit">
			...
			<el-table-column prop="imgfile" label="圖片1" align="center">
				<template slot-scope="scope">
					<div class="demo-image__preview">
						...
						<div slot="error" class="image-slot">
							<a :href="scope.row.ImgZoomIn" target="_blank">
			...
			<el-table-column prop="imgfile" label="圖片2" align="center">
				<template slot-scope="scope">
					<div class="demo-image__preview">
						<el-image
							:src="scope.row.ImgZoomOut"
							:preview-src-list="[scope.row.ImgZoomOut]"
							fit="cover">
						<div slot="error" class="image-slot">
							<a :href="scope.row.ImgZoomOut" target="_blank">
								<i class="el-icon-picture-outline"></i>
							</a>
						</div>
						</el-image>
...
import { getPothole36 } from "@/api/car";
