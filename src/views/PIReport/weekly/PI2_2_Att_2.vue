	...
	methods: {
		...
		async getList() {
			new Promise((resolve, reject) => {
				this.loading = true;
				let startDate = moment(this.reportDate).day(1).format("YYYY-MM-DD");
				...
				getCaseList({
					...
					if (response.data.list.length != 0) {
						...
						this.inputs.listOther = list.filter(l => l.DistressSrc !== "1999交辦案件").reduce((acc, cur) => {
							const itemList = acc.filter(item => item.reportDate == this.formatDate(cur.ReportDate) && item.distressSrc == cur.DistressSrc);
